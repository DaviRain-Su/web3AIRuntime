...
let ensure_deps_exist (actions : action list) : (unit, string) result =
  let ids = List.fold_left (fun acc a -> SSet.add a.id acc) SSet.empty actions in
  let missing =
    actions
...
let topo_sort (actions : action list) : (action list, string) result =
  ...
  List.iter (fun a -> Hashtbl.replace by_id a.id a) actions;

  let indeg = Hashtbl.create 64 in
  List.iter (fun a -> Hashtbl.replace indeg a.id 0) actions;
  List.iter
    (fun a ->
      List.iter
        (fun d ->
  ...
  List.iter (fun a -> if Hashtbl.find indeg a.id = 0 then Queue.add a.id q) actions;
...
let validate (wf : workflow) : (unit, string) result =
  let open Result in
  uniq_ids wf.actions >>= fun () ->
  ensure_deps_exist wf.actions >>= fun () ->
  topo_sort wf.actions |> Result.map (fun _ -> ())
