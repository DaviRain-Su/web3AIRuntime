...
export function printRunTrace(runId) {
    ...
    if (started?.data?.workflow)
        console.log(`workflow: ${started.data.workflow} v${started.data.version ?? ""}`);
    ...
    if (finished?.data)
        console.log(`status: ${finished.data.ok ? "ok" : "failed"}${finished.data.error ? ` (${finished.data.error})` : ""}`);
    ...
    if (finished?.data?.ok === false) {
        ...
        if (lastErr?.tool) {
            ...
            if (lastErr?.data?.error)
                console.log(`lastError: ${lastErr.data.error}`);
        }
        const simRes = [...events].reverse().find((e) => e.type === "tool.result" && e.tool === "solana_simulate_tx");
        const sim = simRes?.data?.result;
        if (sim && sim.ok === false) {
            console.log("simulate: failed");
            if (sim.err)
                console.log(`simulate.err: ${JSON.stringify(sim.err)}`);
            if (Array.isArray(sim.logs) && sim.logs.length) {
                console.log("simulate.logs (tail):");
                for (const line of sim.logs.slice(-10))
                    console.log(`  ${line}`);
            }
        }
    }
...
//# sourceMappingURL=trace_cmd.js.map